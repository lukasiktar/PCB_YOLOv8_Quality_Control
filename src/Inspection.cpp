...
cv::Mat Inspection::inspect(cv::Mat frame, Detection detection) {
	...
	cv::Mat binary_image;
	std::vector<int> thresh_values = { 111,131,151,171,191,201 };		//Set up descrete range of threshold for adaptive thresholding
	...
	for (auto thresh : thresh_values) {
		...
		cv::adaptiveThreshold(grey_image, binary_image, 255, cv::ADAPTIVE_THRESH_GAUSSIAN_C, cv::THRESH_BINARY, thresh, -35);
